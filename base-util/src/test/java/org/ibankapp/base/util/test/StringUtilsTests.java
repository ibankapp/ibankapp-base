...
public class StringUtilsTests {
  ...
  public void testCountOccurrencesOf() {
    ...
    String str = "erowoiueoiur";
    assertTrue("not found = 0",
        StringUtils.countOccurrencesOf(str, "WERWER") == 0);
    assertTrue("not found char = 0",
        StringUtils.countOccurrencesOf(str, "x") == 0);
    assertTrue("not found ws = 0",
        StringUtils.countOccurrencesOf(str, " ") == 0);
    assertTrue("not found empty string = 0",
        StringUtils.countOccurrencesOf(str, "") == 0);
    assertTrue("found char=2", StringUtils.countOccurrencesOf(str, "e") == 2);
    assertTrue("found substring=2",
        StringUtils.countOccurrencesOf(str, "oi") == 2);
    assertTrue("found substring=2",
        StringUtils.countOccurrencesOf(str, "oiu") == 2);
    assertTrue("found substring=3",
        StringUtils.countOccurrencesOf(str, "oiur") == 1);
    assertTrue("test last", StringUtils.countOccurrencesOf(str, "r") == 2);
  }
  ...
  public void testReplace() throws Exception {
    ...
    // Simple replace
    String str = StringUtils.replace(inString, oldPattern, newPattern);
    assertTrue("Replace 1 worked", str.equals("a6AazAfoo77abfoo"));

    // Non match: no change
    str = StringUtils.replace(inString, "qwoeiruqopwieurpoqwieur", newPattern);
    assertSame("Replace non-matched is returned as-is", inString, str);

    // Null new pattern: should ignore
    str = StringUtils.replace(inString, oldPattern, null);
    assertSame("Replace non-matched is returned as-is", inString, str);

    // Null old pattern: should ignore
    str = StringUtils.replace(inString, null, newPattern);
    assertSame("Replace non-matched is returned as-is", inString, str);
  }
  ...
  public void testQuoteIfString() {
    ...
    assertEquals(5, StringUtils.quoteIfString(5));
  ...
  public void testStripFilenameExtension() {
    ...
    assertEquals("/home/user/.m2/settings/myfile",
        StringUtils.stripFilenameExtension("/home/user/.m2/settings/myfile"));
    assertEquals("/home/user/.m2/settings/myfile",
        StringUtils.stripFilenameExtension("/home/user/.m2/settings/myfile."));
    assertEquals("/home/user/.m2/settings/myfile",
        StringUtils.stripFilenameExtension("/home/user/.m2/settings/myfile.txt"));
  }
  ...
  public void testCleanPath() {
    ...
    assertEquals("file:///c:/path/to/the%20file.txt",
  ...
  public void testCommaDelimitedListToStringArraySingleString() {
    // Could read these from files
    String str = "woeirqupoiewuropqiewuorpqiwueopriquwopeiurqopwieur";
    String[] sa = StringUtils.commaDelimitedListToStringArray(str);
  ...
  public void testParseLocaleWithMultiSpecialCharactersInVariant() throws Exception {
    ...
    assert locale != null;
    assertEquals("Multi-valued variant portion of the Locale not extracted correctly.", variant,
        locale.getVariant());
  }
  ...
  @Test  // SPR-3671
  public void testParseLocaleWithMultiValuedVariant1()
      throws Exception {
    String variant = "proper northern";
  ...
  @Test  // SPR-3671
  public void testParseLocaleWithMultiValuedVariant2()
      throws Exception {
    String variant = "proper_northern";
  ...
  public void testParseLocaleWithVariantContainingCountryCode() {
    ...
    Locale locale = StringUtils.parseLocaleString(localeString);
    assert locale != null;
    assertEquals("Variant containing country code not extracted correctly", variant,
        locale.getVariant());
  }
